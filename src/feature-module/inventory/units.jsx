...
const GetData = async () => {
  let url = `/api/GetMaster?MasterType=8&ParentID=0`;
  try {
  //   setLoading(true);
    const { res, got } = await callFetch(url, "GET");
    // Handle response data
...
React.useEffect(() => {
  ...
  const showConfirmationAlert = (code) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        const message = await unitDelete(code);
        if (message) {
          MySwal.fire({
  ...
  return (
    ...
    <div className="page-wrapper">
      ...
      <AddUnit handleSaveMaster={handleSaveMaster} formData={formData} handelInputChange={handelInputChange}/>
      <EditUnit handleSaveMaster={handleSaveMaster} formData={formData} handelInputChange={handelInputChange} show={showModal}
