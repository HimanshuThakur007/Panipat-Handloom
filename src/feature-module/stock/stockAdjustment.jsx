...
const StockAdjustment = () => {
  ...
  const loadStockTable = async () => {
    let url = `/api/GetPhysicalDetails`;
    try {
      setLoading(true);
      const { res, got } = await callFetch(url, "GET");
      if (res.status === 200) {
        const loadData = got.data;
        setStockTableData(loadData); 
        setLoading(false);
      }
    } catch (error) {
      setLoading(false);
      ...
      render: (record) => (
        <td className="action-table-data">
          <div className="edit-delete-action">
            ...
            <Link
              className="me-2 p-2"
              to={`/stock/${record.masterType}`}
              state={{ code: record.vchCode, mt:record.masterType }}
              // data-bs-toggle="modal"
  ...
  return (
    <div className="page-wrapper">
      ...
      {/* <BulkUploadModal downloadExcel={downloadExcel}/> */}
    </div>
  );
};
